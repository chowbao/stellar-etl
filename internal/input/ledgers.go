...
// LCMToHistoryArchiveLedgerAndLCM converts a LedgerCloseMeta to a HistoryArchiveLedgerAndLCM
func LCMToHistoryArchiveLedgerAndLCM(lcm xdr.LedgerCloseMeta) utils.HistoryArchiveLedgerAndLCM {
	...
	return utils.HistoryArchiveLedgerAndLCM{Ledger: ledger, LCM: lcm}
}
...
func GetLedgers(start, end uint32, limit int64, env utils.EnvironmentDetails, useCaptiveCore bool) ([]utils.HistoryArchiveLedgerAndLCM, error) {
	...
	for seq := start; seq <= end; seq++ {
		...
		ledgerSlice = append(ledgerSlice, LCMToHistoryArchiveLedgerAndLCM(lcm))
