...
func TestExportLedger(t *testing.T) {
	tests := []CliTest{
		...
		{
			Name:    "single ledger",
			Args:    []string{"export_ledgers", "-s", "30822015", "-e", "30822015", "-o", GotTestDir(t, "single_ledger.json")},
			Golden:  "single_ledger.golden",
			WantErr: nil,
		},
		{
			Name:    "10 ledgers",
			Args:    []string{"export_ledgers", "-s", "30822015", "-e", "30822025", "-o", GotTestDir(t, "10_ledgers.json")},
			Golden:  "10_ledgers.golden",
			WantErr: nil,
		},
		{
			Name:    "range too large",
			Args:    []string{"export_ledgers", "-s", "30822015", "-e", "30822025", "-l", "5", "-o", GotTestDir(t, "large_range_ledgers.json")},
			Golden:  "large_range_ledgers.golden",
			WantErr: nil,
		},
		{
			Name:    "range from 2024",
			Args:    []string{"export_ledgers", "-s", "52929555", "-e", "52929960", "-o", GotTestDir(t, "2024_ledgers.json")},
